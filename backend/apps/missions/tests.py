...
class MissionTests(APITestCase):
    ...
    def test_get_specific_mission(self):
        ...
        """
        url = reverse(
            "missions_views-detail", args=[self.mission1.id]
        )  
        response = self.client.get(url, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class DriverTests(APITestCase):
    def setUp(self):
        ...
        self.vehicle_type = Vtype.objects.create(name="Ambulance",description="Ambulance type vehicle")
        ...
        self.driver1 = Driver.objects.create(
            mission_id=self.mission1,
            vehicle_id=self.vehicle2,
            driver_id=self.user
        )
    ...
    def test_view_all_drivers(self):
        ...
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)  # Assuming there are two drivers

        # Check if the response data contains the correct drivers
